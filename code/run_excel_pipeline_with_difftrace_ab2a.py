...
"""ab2-A Excel pipeline: 用 ab2-A 评估结果回填 execution_result_ab2a.xlsx，并执行 full-coverage 传递。

- 输入: dataset/llm_vulnerability_presence_eval_ab2_A.json
- Excel: dataset/execution_result_ab2a.xlsx
- 步骤:
  1) 清理 / 初始化 LLM_Verdict / Aligned_llm 列;
  2) 按 (CVE_ID, Version) 聚合 ab2-A 的 predicted_label，回填为 not affected_intro / affected_intro；
  3) 调用 result_analysis_full_coverage_ab2a.py，对版本链做前向/后向 full-coverage 传递与评估。
"""
...
EVAL_JSON_PATH = DATASET_ROOT / "llm_vulnerability_presence_eval_ab2_A.json"
EXCEL_PATH = DATASET_ROOT / "execution_result_ab2a.xlsx"
...
def apply_predictions_to_execution_result_ab2a(
...
def main() -> None:
    ...
    if not eval_path.is_file():
        raise FileNotFoundError(f"找不到 ab2-A 评估文件: {eval_path}")
    if not excel_path.is_file():
        raise FileNotFoundError(f"找不到 execution_result_ab2a.xlsx: {excel_path}")

    details = load_eval_details(eval_path)
    aggregated_predictions, aggregation_stats = aggregate_difftrace_predictions(details)

    print("=" * 100)
    print("DiffTrace ab2-A → execution_result_ab2a.xlsx 回填 + full-coverage 流水线")
    print("=" * 100)
    print(f"ab2-A 评估文件: {eval_path}")
    ...
    stats = apply_predictions_to_execution_result_ab2a(excel_path, aggregated_predictions)

    print("\n--- ab2-A 回填结果 (execution_result_ab2a.xlsx) ---")
    ...
    # 调用 ab2-A full-coverage 版本链推理
    from result_analysis_full_coverage_ab2a import main as full_coverage_main

    print("\n>>> 继续执行 ab2-A full-coverage 版本链传递与评估 ...")
    full_coverage_main()
