...
all_info = []
with open(os.path.join(DATASET_DIR, "dataset_o1.json")) as f:
    all_info = json.load(f)
# Diagnostic prints commented out
# print(f"DATASET_DIR={DATASET_DIR}", flush=True)
# print(f"REPOS_DIR={REPOS_DIR}", flush=True)
# print(f"CWD={CWD}", flush=True)
# print(f"len(all_info):{len(all_info)}", flush=True)

# show a sample of the first few entries to help debug fast exits
# for i, info_sample in enumerate(all_info[:5]):
#     repo_name_sample = info_sample.get("repo_name") if isinstance(info_sample, dict) else None
#     repo_path_sample = os.path.join(REPOS_DIR, repo_name_sample) if repo_name_sample else None
#     exists = os.path.exists(repo_path_sample) if repo_path_sample else False
#     print(f"sample[{i}] repo_name={repo_name_sample} repo_path_exists={exists}", flush=True)
deal_cids = []
errs_info = []
dealed_infos = []
pipeline = None
    ...
    for info in all_info:
        ...
        if not os.path.exists(repo_path):
            try:
                ...
                print(f"Created missing repo directory: {repo_path}", flush=True)
            except Exception as e:
                print(f"Failed to create repo directory {repo_path}: {e}", flush=True)
                # if we cannot create the directory, skip this item
                continue
        ...
        try:
            ...
            print("现在的北京时间是：", beijing_time.strftime("%Y-%m-%d %H:%M:%S"))
