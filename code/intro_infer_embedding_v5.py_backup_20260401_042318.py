...
OUTPUT_PATH = os.path.join(BASE_PATH, "result/intro_contrastive_final_h.json")

# 是否使用LLM确认（基于代码特征辅助判断，不使用硬编码CVE列表）
USE_LLM_CONFIRMATION = True
LLM_MAX_TOKENS = 500  # 限制LLM调用token数
CVE_TIMEOUT = 600  # 每个CVE处理超时时间（秒）
MAX_LLM_CALLS_PER_CVE = 5  # 每个CVE最多调用LLM的次数（规则筛选后） - 减少以降低误报
...
def analyze_candidate_commit(commit_context, fix_security_patterns, fix_security_measures, base_tag=None):
    ...
    confidence = 0.45  # Base confidence - revert to original value
    confidence += min(len(critical_vulnerability_patterns) * 0.03, 0.15)  # Critical patterns bonus - revert to original
    confidence = min(confidence, 0.60)  # Rules part max 0.60, LLM will make final decision
...
def process_cve(cve_id, base_tag=None):
    ...
    try:
        ...
        for candidate in candidates_scores:
            ...
            threshold = 0.30  # Revert to original threshold to reduce false positives
