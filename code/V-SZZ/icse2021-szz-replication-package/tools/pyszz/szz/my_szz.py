...
class MySZZ(AbstractSZZ):
    ...
    * ignore_revs_file_path
    ...
    def __init__(self, repo_full_name: str, repo_url: str, repos_dir: str = None, use_temp_dir: bool = True, ast_map_path = None):
        ...
        super().__init__(repo_full_name, repo_url, repos_dir, use_temp_dir)
        self.repo_url = repo_url  # <<--- 必须添加这一行
        self.repo_full_name = repo_full_name # 确保后续 mapping_cmd 也能读到
        self.ast_map_path = ast_map_path
    ...
    def _is_valid_git_url(self, url: str) -> bool:
        ...
        try:
            result = urlparse(url)
            # 校验协议是否为 http, https, 或 ssh 常用格式
            return all([result.scheme, result.netloc]) or url.startswith("git@")
        except:
            return False
    ...
    def map_modified_line_java(self, blame_entry, blame_file_path):
        ...
        url_to_use = self.repo_url if self.repo_url else log.error("repo_url is not set in MySZZ instance.")

        if not self._is_valid_git_url(self.repo_url):
            log.error(f"Invalid git URL format: {self.repo_url}")
            raise ValueError(f"The provided repo_url '{self.repo_url}' is not a valid Git URL.")
        

        # TODO
        # url_to_use = "/home/xinweimao/alv_evaluate/myResearch/workspace/code/V-SZZ/repo/junit4"

        mapping_cmd = f"java -jar ASTMapEval.jar -p {{project}} -c {{commit_id}} -o {{output}} -f {{file_path}} -r {url_to_use}"


    
        # mapping_cmd = (
        #     "java -Dgit.url=https://github.com/junit-team/junit4.git "
        #     "-Durl=https://github.com/junit-team/junit4.git "
        #     "-Drepo.url=https://github.com/junit-team/junit4.git "
        #     "-jar ASTMapEval.jar -p {project} -c {commit_id} -o {output} -f {file_path}"
        # )
        ...
        if os.path.exists(mapping_db_file):
            ...
            # print(3)
            # print(mapping_cmd.format(project=self.repo_full_name, commit_id=commit_id, output=os.path.join(ast_map_temp, "tmp.json"), file_path=file_path))
            subprocess.check_output(mapping_cmd.format(project=self.repo_full_name, commit_id=commit_id, output=os.path.join(ast_map_temp, "tmp.json"), file_path=file_path), cwd=self.ast_map_path, shell=True).decode('utf-8', errors='ignore')
