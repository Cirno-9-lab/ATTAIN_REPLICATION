...
MAX_LLM_CALLS_PER_CVE = 3  # 每个CVE最多调用LLM的次数（规则筛选后）
...
def analyze_candidate_commit(commit_context, fix_security_patterns, fix_security_measures, base_tag=None):
    ...
    # Base confidence (rules part) - very conservative
    confidence = 0.40  # Base confidence
    confidence += min(len(critical_vulnerability_patterns) * 0.02, 0.10)  # Critical patterns minor bonus
    confidence = min(confidence, 0.50)  # Rules part max 0.50, LLM will make final decision
...
- If you look at the ADDED LINES, you can identify the root cause of the vulnerability
...
- Does the ADDED CODE contain security flaws that were NOT present before?

### 2. Code Semantic Analysis
- What is the main functionality of the ADDED CODE?
...
Important: Focus on ADDED CODE semantics and vulnerability introduction characteristics. Do NOT be influenced by the CVE ID."""
        ...
        # Debug: 打印LLM原始响应（前500字符）
        print(f"  🔬 LLM原始响应（前500字符）: {response_text[:500]}")

        # Parse LLM response
        if "ERROR" in response_text or "STOP_SIGNAL" in response_text:
            print(f"  ❌ LLM call failed: {response_text[:100]}")
        ...
        try:
            ...
            print(f"  ✓ LLM result: is_intro={is_intro}, confidence={llm_confidence:.2%}")
...
def process_cve(cve_id, base_tag=None):
    ...
    for candidate in candidates_scores:
        ...
        # Use lower threshold since LLM has performed semantic judgment
        # LLM (70% weight) dominates the decision making
        threshold = 0.40  # Lower threshold to increase recall
