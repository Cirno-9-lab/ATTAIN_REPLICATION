...
def main():
    ...
    for cve_id, entries in analysis_data.items():
        for entry in entries:
            ...
            # 优先判定：外层 true_result 是否为 yes
            if str(entry.get("true_result", "")).strip().lower() == "yes":
                is_passed = True
            
            # 如果外层未判定，进入 analysis 细节
            if not is_passed:
                ...
                for commit_id, files_dict in analysis_section.items():
                    for f_path, f_info in files_dict.items():
                        if isinstance(f_info, dict):
                            # 提取判定标记 (verdict 或 true_result)
                            v_val = str(f_info.get("verdict", f_info.get("true_result", ""))).strip().lower()
                            # 提取得分
                            score = float(f_info.get("score", f_info.get("msg_score", 0)))
                            
                            # 满足任一条件即视为补丁已引入
                            if v_val == "yes" or score >= FIXED_THRESHOLD:
                                is_passed = True
                                break
                    if is_passed: break
    ...
    print("\n" + "═"*45)
    print(f"🚀 最终评估 (逻辑：层级判定 + 阈值 {FIXED_THRESHOLD})")
    print("─" * 45)
