...
COMMIT_INFO_DIR = os.path.join(BASE_PATH, "commit_info")
...
def load_fix_code_context(cve_id, fix_commits_files):
    try:
        ...
        from security_features import SECURITY_PATTERNS
        security_patterns = []
...
def load_intro_commits_with_code(cve_id, base_tag, head_tag):
    """
    加载候选commits及其代码内容

    从intro_context.json获取候选列表，从commit_info加载代码
    """
    try:
        # 1. 加载intro_context.json获取候选列表
        ...
        for version_pair in intro_data[cve_key]:
            if version_pair.get("base_tag") == base_tag:
                ...
                for item in filter_summary:
                    candidate_shas.append(item["sha"])

        print(f"    从intro_context找到 {len(candidate_shas)} 个候选SHA")

        # 2. 从commit_info加载代码
        commit_info_file = os.path.join(COMMIT_INFO_DIR, f"{cve_id.lower()}.json")
        with open(commit_info_file, 'r') as f:
            commit_info_data = json.load(f)

        intro_commits = {}
        for version_pair in commit_info_data.get(cve_id.lower(), []):
            if version_pair.get("base_tag") == base_tag:
                commit_info = version_pair.get("commit_info", {})
                for sha, info in commit_info.items():
                    if sha in candidate_shas:
                        intro_commits[sha] = info

        print(f"    从commit_info匹配到 {len(intro_commits)} 个候选的代码")
...
def generate_candidates_from_info(intro_commits, fix_commits):
    """从commit_info生成候选commits"""
    candidates = []
...
def test_single_cve(cve_id, base_tag, head_tag, expected_result, llm_client, engine):
    ...
    # 加载候选commits（新方法）
    intro_commits = load_intro_commits_with_code(cve_id, base_tag, head_tag)
    if not intro_commits:
        print("⚠️  未找到候选commits")
        return (expected_result == "should_not_find", {"reason": "no_intro_commits"})

    # 生成候选
    candidates = generate_candidates_from_info(intro_commits, fix_commits)
    if not candidates:
        print("⚠️  未找到候选commits")
