...
ENTRY_SOURCE = WORKSPACE / 'tmp/cve_2025_58056_unified_time_scan_rq3.json'
SNAPSHOT_SOURCE = WORKSPACE / 'tmp/cve_2025_58056_matrix_latest.json'
OUTPUT_PATH = WORKSPACE / 'tmp/CVE-2025-58056_version_tree.svg'
...
def load_payload():
    ...
    def build_version_tree_from_matrix(snapshot: dict):
        matrix = snapshot.get('branch_matrix') or []
        branches_by_key = {}
        for row in matrix:
            if not row:
                continue
            for cell in row:
                if not cell:
                    continue
                key = str(cell.get('branch_key') or 'main')
                row_index = int(cell.get('row') or 0)
                info = branches_by_key.setdefault(
                    key,
                    {
                        'branch_key': key,
                        'branch_row': row_index,
                        'branch_type': cell.get('branch_type'),
                        'branch_depth': cell.get('branch_depth'),
                        'line_key': cell.get('line_key'),
                        'fork_anchor_version': cell.get('fork_anchor_version'),
                        'versions': [],
                    },
                )
                if row_index < info.get('branch_row', row_index):
                    info['branch_row'] = row_index
                for field in ('branch_type', 'branch_depth', 'line_key', 'fork_anchor_version'):
                    if cell.get(field) is not None:
                        info[field] = cell.get(field)
        ...
        branches.sort(key=lambda b: int(b.get('branch_row') or 0))
        return branches
    ...
    if not version_tree and snapshot_entry and snapshot_entry.get('branch_matrix'):
        version_tree = build_version_tree_from_matrix(snapshot_entry)
        merged_entry['version_tree'] = version_tree
...
def render_svg(version_tree, entry: dict, output_path: Path):
    ...
    parts = [
        ...
        f'<text class="subtitle" x="{left_margin}" y="58">artifact: {escape(entry.get("artifact", "unknown"))} | branches: {len(branches)} | rule: hotfix branches only keep fork links to anchors</text>',
        f'<text class="subtitle" x="{left_margin}" y="80">布局语义：每一行是一条真实 branch；彩色连线只表示 fork 派生关系，不表示版本 compare pair。</text>',
        f'<text class="subtitle" x="{left_margin}" y="100">角标：红色 A = appears，绿色 N = not appears；橙色节点表示同 core 且一天内发布的 hotfix 分支。</text>',
    ]

    legend_y = 126
    legend_x = max(left_margin + 540, width - 860)
    legend_items = [
        ('stable', 'stable'),
