...
ENTRY_SOURCE = WORKSPACE / 'tmp/cve_2022_42003_unified_time_scan.json'
SNAPSHOT_SOURCE = WORKSPACE / 'tmp/cve_2022_42003_matrix_latest.json'
OUTPUT_PATH = WORKSPACE / 'tmp/CVE-2022-42003_version_tree.svg'
...
def render_svg(version_tree, entry: dict, output_path: Path):
    ...
    parts = [
        ...
        f'<text class="subtitle" x="{left_margin}" y="58">artifact: {escape(entry.get("artifact", "unknown"))} | branches: {len(branches)} | rule: only compare adjacent versions inside the same branch</text>',
        f'<text class="subtitle" x="{left_margin}" y="80">布局语义：每一行是一条真实 branch；彩色连线只表示 fork 派生关系，不表示版本 compare pair。</text>',
        f'<text class="subtitle" x="{left_margin}" y="100">角标：红色 A = appears，绿色 N = not appears；灰色节点表示 Excel unresolved 但仍顺接在当前分支。</text>',
    ]

    legend_y = 126
    legend_x = max(left_margin + 660, width - 760)
    legend_items = [
        ('stable', 'stable'),
